  ...
  <div class="members-page fade-in">
    <div class="page-header d-flex justify-content-between align-items-center">
      ...
      <button class="btn btn-primary-gradient" @click="openCreate">
        <i class="bi bi-plus-lg me-1"></i>{{ $t('members.addMember') }}
      </button>
    </div>
    <div class="row g-3">
      <div v-for="member in members" :key="member.id" class="col-md-4 col-lg-3">
        <div class="stat-card text-center">
          ...
          <span class="badge bg-secondary mb-2">{{ $t('members.roles.' + member.role) }}</span>
          <div>
    ...
    <div v-if="showModal" class="vue-modal-backdrop" @mousedown.self="showModal = false">
      <div class="vue-modal">
        <div class="modal-header">
          <h5 class="modal-title">{{ editingId ? $t('members.editMember') : $t('members.addMember') }}</h5>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <select v-model="form.role" class="form-select" required>
                <option value="" disabled>- {{ $t('common.role') }} -</option>
                <option value="father">{{ $t('members.roles.father') }}</option>
                <option value="mother">{{ $t('members.roles.mother') }}</option>
    ...
    <div v-if="showDeleteModal" class="vue-modal-backdrop" @mousedown.self="showDeleteModal = false">
      <div class="vue-modal" style="max-width:420px">
        <div class="modal-header border-0 pb-0">
          <h5 class="modal-title text-danger"><i class="bi bi-exclamation-triangle me-2"></i>{{ $t('common.delete') }} {{ $t('common.member') }}</h5>
