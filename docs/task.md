...
## Part 13 — WhatsApp Push Notifications (whatsapp-web.js)
- `[x]` Task 77: Create WhatsApp bot repository (`famfin-whatsapp-bot`) and configure package.json
- `[x]` Task 78: Implement WhatsApp bot Express server using `whatsapp-web.js`
- `[x]` Task 79: Create database migration `000025_add_whatsapp_numbers.sql` to support member phone numbers
- `[x]` Task 80: Create database migration `000026_shopping_plan_webhook.sql` using pg_net to trigger the Heroku bot
- `[x]` Task 81: Update `MemberModal.vue` and `MembersPage.vue` with WhatsApp Number inputs and Notification toggles
